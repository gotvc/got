...
func TestFork(t *testing.T) {
	...
	require.NoError(t, repo.History(ctx, &gotcore.CommitExpr_Mark{Name: "branch2"}, func(_ gotfs.Ref, _ gotcore.Commit) error {
		return nil
	}))
...
func checkFileContent(t testing.TB, wc *WC, p string, r io.Reader) {
	...
	pr, pw := io.Pipe()
	go func() {
		err := wc.repo.Cat(ctx, gotcore.CommitExpr_Mark{Name: getHead(t, wc)}, p, pw)
		if err != nil {
...
func exists(t testing.TB, wc *WC, p string) bool {
	ctx := testutil.Context(t)
	var found bool
	err := wc.repo.Ls(ctx, gotcore.CommitExpr_Mark{Name: getHead(t, wc)}, path.Dir(p), func(ent gotfs.DirEnt) error {
...
func countCommits(t testing.TB, repo *gotrepo.Repo, branchName string) int {
	ctx := testutil.Context(t)
	var count int
	repo.History(ctx, gotcore.CommitExpr_Mark{Name: branchName}, func(_ gotfs.Ref, _ gotcore.Commit) error {
		count++
		return nil
	})
	return count
