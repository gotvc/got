...
func NewIWriter(mach *Machine) IWriter {
	// the first 65 bytes are a ref to the constants, and the depth.
	buf := make([]byte, 65)
	return IWriter{
		mach:  mach,
...
func (w *IWriter) ILen() int {
	return (len(w.out) - (gdat.RefSize + 1)) / 4
}
...
func (w *IWriter) postValues(ctx context.Context, swo stores.RW) (gotkv.Root, error) {
	b := w.gotkv.NewBuilder(swo)
...
func (w *IWriter) Flush(ctx context.Context, swo stores.RW) (gdat.Ref, error) {
	ref, err := w.postValues(ctx, swo)
	if err != nil {
		return gdat.Ref{}, err
	}
	copy(w.out[:gdat.RefSize+1], ref.Marshal(nil))
...
func parseFunctionBody(ctx context.Context, gm *gdat.Machine, s stores.Reading, data []byte) (*Expr, error) {
	...
	if err := kvroot.Unmarshal(data[:gdat.RefSize+1]); err != nil {
