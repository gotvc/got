...
type SnapExpr = gotcore.SnapExpr

// ViewSnapshot calls fn to view a Snapshot in read-only mode
func (r *Repo) ViewSnapshot(ctx context.Context, se SnapExpr, fn func(*gotcore.ViewCtx) error) error {
	space, err := r.GetSpace(ctx, se.GetSpace())
	if err != nil {
		return err
	}
	return space.Do(ctx, false, func(st gotcore.SpaceTx) error {
		return gotcore.ViewSnapshot(ctx, st, se, fn)
	})
}
...
func (r *Repo) DebugFS(ctx context.Context, se gotcore.SnapExpr, w io.Writer) error {
	return r.ViewSnapshot(ctx, se, func(vctx *gotcore.ViewCtx) error {
		return gotfs.Dump(ctx, vctx.Stores[1], vctx.Root.Payload.Root, w)
	})
}

func (r *Repo) DebugKV(ctx context.Context, se gotcore.SnapExpr, w io.Writer) error {
	return r.ViewSnapshot(ctx, se, func(vctx *gotcore.ViewCtx) error {
