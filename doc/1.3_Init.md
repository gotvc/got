...
- `.got/wc-config` the [Working Copy](./2.2_Working_Copy.md) configuration file.
  The existence of this file means the directory is also a Working Copy.
...
This is saying that any new Commits will be stored in a Mark called `master`.
...
Got is telling us that, if we were to commit the changes into a Commit, we would be creating a new file called "myfile.txt"
...
This tells us some information about the commit that we just created.
1. The Commit number (or critical distance to the sinks) is 0.
That is because this Commit is the first one in this history, and has no parents.
2. The Commit ID is `7VHDkN79AOlQPt_O_82i5kwwLWrxB_b157Ra3sWE7zN`; yours will be different.
3. The hash of the filesystem root is `f2GpAom310PLtxx7KDV0-vsH0Nvzji1Og2n1NMqemYJ`; yours will be different.
4. The Commit has no parents
5. When the Commit was created, this is always displayed in the local timezone.
6. The ID of the user that created the Commit
7. Everything else is arbitrary metadata, it is stored as JSON in the Commit.
...
The filesystem committed in the most recent Commit, which the Mark `master` points to, contains a single file, called `myfile.txt`.
